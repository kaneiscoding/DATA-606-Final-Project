...
We can observe a chi-squared distribution for the filtered ans scaled value of "popularity".
...
```{r}
# Remove one record with date value of 1900-01-01 to clean up the decade plots
...
Using K-fold cross validation we obtained a Misclassification Rate of 24.09%.
...
vif(model_fit)
```
Using the VID function we checked for Multicolinearity, no major issues found, the only variable that has a high VIF value is "energy", as its less than 5 no changes are needed. 
...
plot(tree.class)
text(tree.class, cex=1.0, col="black")
```
...
plot(cv.class$size, cv.class$dev, type='b', cex.main=1.0, cex.lab=1.0, cex.axis=1.0)
```
...
- LDA model was not valid based on assumptions​

- Regression Tree only capable of predicting popularity scores of 20,30,40,50 with RMSE of 15. 'Hit' defined as >42.  Would like to see some terminal nodes closer to 100.​

- Classification tree and Logistic Regression produced similar misclassification rates​
  - Unbalanced: ~24%​
  - Balanced: ~33%​

- Balancing the classification tree improved the feature space beyond loudness and explicitness, but had poorer performance overall. ​


# 5 Conclusion

- Logistic regression was our most useful model​

- Balancing our target variable in classification models decreased performance! But told us more?​

- Some concern that misclassification percentage matched class proportions in unbalanced models​

- Potentially useful information for musicians, producers, and industry professionals ​

- Further analysis would require scraping actual song data that is not produced by Spotify,  providing more granular data.​

- Further analysis to be conducted into predictability of genre from these features​
...
```{r remove artists with multiple genres}
...
```{r merge tracks with artists}
#inner join so only the artists with single genre remain alongsige the track data
genre_tracks <- merge(x=artist_genres, y=track_artists, by.x="id", by.y="id_artists") %>%  dplyr::select(id:time_signature)
...
Next, the data is grouped by genre as a stata to facilitate splitting for train and test. 150000 observations were used for training to roughly target 75%  (actually 73%). Below, the count of the strata can be seen.
...
The mislclassification rate for the tree on the test data is found as shown below:
...
mis
```
The performance of the tree is 0.4181 misclassification. It can be seen that the tree does not preduct many of the genres well. Genres such as "blues", "classicial" and country have no correct predictions. This could be related to the imbalanced dataset (high pop), or difficulty in drawing clear lines between the genres in general. 

Although the assumptions for LDA were not met, as seen in the main body of the report, the LDA exercise was performed for completenes:
